...
const ProductTableRow = ({ product, onUpdateStatus }) => {
  ...
  return (
    <tr key={product.id} className="text-left">
      <td className="w-20">
        <img src={Avacado} alt={`Product: ${product.name}`} />
      </td>
      ...
      <td className="px-6 py-4 text-right flex justify-end gap-x-4 bg-gray-50">
        <button
          className="text-lg px-6 py-4"
          onClick={() => handleStatusUpdate(ProductStatus.APPROVED)}
        >
          ✓
        </button>
        <button
          className="text-lg px-6 py-4 "
          onClick={() => handleStatusUpdate(ProductStatus.MISSING)}
        >
          X
        </button>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (!productsAddedRef.current) {
      // Add products to the Redux store
    ...
    <div className="flex flex-col rounded-2xl bg-white divide-x-2 py-5 px-14 w-full justify-center mb-4">
      ...
      <div className="overflow-x-auto rounded-t-lg mt-4">
        <table className="w-full text-sm text-left text-gray-500 border-t-2 border-r-2 border-l-0 border-collapse">
          ...
          <tbody>{mapProductTableRows()}</tbody>
        </table>
      </div>
    </div>
  );
